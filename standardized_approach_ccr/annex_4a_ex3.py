...
df['hedging_set'] = ['crude_oil','crude_oil','silver']
...
df['delta_i'] = [1,-1,1]
df['MF_i'] = [math.sqrt(float(9)/12),1,1]

#aggregate by hedging_set
df['hedging_set_cd'] = df['hedging_set'] 
print df
...
df_hedging_set['addon_systematic'] = df_hedging_set.apply(lambda x: (x['rho_k'] * x['addon_k'])**2, axis=1)
df_hedging_set['addon_idiosyncratic'] = df_hedging_set.apply(lambda x: (1 - x['rho_k']**2) * x['addon_k']**2, axis=1)
df_hedging_set['addon'] = df_hedging_set.apply(lambda x: math.sqrt(x['addon_systematic'] + x['addon_idiosyncratic']), axis=1)

df_asset_class = pd.DataFrame(index=['commodity'])
df_asset_class['addon'] = sum(df_hedging_set['addon'])
